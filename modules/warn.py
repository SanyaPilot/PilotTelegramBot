...
async def warn(message: Message, user_obj=None):
    ...
    try:
        ...
        if check_perms and message.from_user.id == user.id:
            await message.reply(trans['warn']['same_usr_err'][0])
...
@dp.message_handler(commands='resetwarns')
async def reset_warns(message: Message):
    ...
    try:
        ...
        if check_perms and message.from_user.id == user.id:
            await message.reply(trans['warn']['same_usr_err'][1])
            logger.warning(f"{message.chat.full_name}: User {username} wanted to warn himself")
            return
        ...
        if not user.id == me.id:
            ...
            if data:
                session.query(Warns).filter_by(user_id=user.id, chat_id=message.chat.id).delete()
                session.commit()
                await message.reply(text=trans['warn']['reset'].format(name), parse_mode='HTML')
            else:
                await message.reply(trans['warn']['no_warns_err'])
