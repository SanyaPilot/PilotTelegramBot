...
from init import dp, tw, bot, Chats, session
                    ...
                    if not message.from_user.id == user.id:
                        if member2.status == 'creator' or member2.status == 'administrator':
                            if '--force' in message.get_args():
                                ...
                                logger.warning(
                                    f"{message.chat.full_name}: User {username} not --force flag")
                                return
                        ...
                        if duration != datetime.timedelta(hours=999999):
                            if not duration < datetime.timedelta(seconds=30):
                                await bot.restrict_chat_member(chat_id=message.chat.id,
                                                               ...
                                                               until_date=duration)
                                logger.info(f"{message.chat.full_name}: User {username} was muted")
                                ...
                                logger.info(f"{message.chat.full_name}: Can't mute {username} for less than 30 seconds")
                        else:
                            await bot.restrict_chat_member(chat_id=message.chat.id,
                                                           user_id=user.id,
                                                           can_send_messages=False)
                        ...
                        logger.warning(
                            f"{message.chat.full_name}: User {message.from_user.full_name} wanted to mute himself")
                else:
                    await message.reply(trans['global']['errors']['affect_on_bot'])
                    logger.warning(
                        f"{message.chat.full_name}: User {message.from_user.full_name} wanted to mute bot")
            else:
                ...
                if not user.id == me.id:
                    if not message.from_user.id == user.id:
                        ...
                        if member2.status == 'restricted':
                            ...
                            logger.info(f"{message.chat.full_name}: {username} unmuted")
                        else:
                            await message.reply(trans['mute']['user_not_muted'])
                            logger.warning(f"{message.chat.full_name}: {username} not muted")
                    else:
                        await message.reply(trans['global']['errors']['affect_on_bot'])
                        logger.warning(
